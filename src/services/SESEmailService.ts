...
export class SESEmailService {
    ...
    constructor(config: SESConfig = {}) {
        ...
        this.templatePath = config.templatePath || path.join(process.cwd(), 'email-templates');
    ...
    async sendEmailWithAttachments(emailData: EmailWithAttachments): Promise<EmailResult> {
        try {
            ...
            const params: SendRawEmailCommandInput = {
                RawMessage: {
                    Data: new TextEncoder().encode(rawEmail), // ✅ convert string → Uint8Array
                },
    ...
    private async loadTemplate(templateName: string): Promise<string> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            throw new Error(`Template '${templateName}' not found: ${errorMessage}`);
        }
    }
