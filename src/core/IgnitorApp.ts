...
export class IgnitorApp {
    ...
    private initializeCore(): void {
        ...
        const allowedOrigins = [
            'http://localhost:5173',
            'http://172.16.100.89:5173',
            'http://172.16.40.56:5173',
            'http://192.168.10.84:5173',
            'http://172.16.40.56',
            'http://172.16.200.114:5173',
            'https://onschedule.ca',
            'https://www.onschedule.ca',
            'https://api.onschedule.ca',
            'https://superr-admin-dan.vercel.app',
            'https://onschedule-admin-dashboard.vercel.app'
        ];

        // https://api.onschedule.ca/api/payment/stripe/webhook
        ...
        this.app.post(
            ...
            async (req: Request, res: Response) => {
                ...
                if (!authToken) {
                AppLogger.error('TWILLO_AUTH_TOKEN not found in config');
                return res.status(500).send('Server misconfigured');
                }

                const twilioSignature = req.headers['x-twilio-signature'] as string;
                if (!twilioSignature) {
                AppLogger.warn('Missing X-Twilio-Signature header');
                return res.status(400).send('Bad Request');
                }

                const callbackUrl = process.env.TWILIO_STATUS_CALLBACK_URL;
                if (!callbackUrl) {
                AppLogger.error('TWILIO_STATUS_CALLBACK_URL not set in .env');
                return res.status(500).send('Server misconfigured');
                }
                ...
                if (!isValid) {
                AppLogger.warn('Invalid Twilio signature - request rejected', {
                    ip: req.ip,
                    url: callbackUrl,
                });
                ...
                AppLogger.info('Twilio SMS Status Update', {
                MessageSid,
                To,
                ...
                try {
                await this.context.prisma.sMSLog.upsert({
                ...
                AppLogger.error('Failed to save SMS status to DB', {
                    error: error.message,
                    MessageSid,
                });
                }
