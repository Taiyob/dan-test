...
//     try {
//       return await this.getModel().findFirst({ where: filters, include });
//     } catch (error) {
...
//   protected async updateById(id: string | number, data: TUpdateInput, include?: any): Promise<TModel> {
//     try {
...
//     try {
//       const count = await this.getModel().count({ where: filters });
//       return count > 0;
//     } catch (error) {
...
//   private buildPaginationResult<T>(data: T[], total: number, pagination: PaginationOptions): PaginationResult<T> {
...
export interface BaseServiceOptions {
  ...
  defaultPageSize?: number;
  maxPageSize?: number;
  enableSSE?: boolean; // New option for SSE support
}
...
export abstract class BaseService<TModel = any, TCreateInput = any, TUpdateInput = any> {
   ...
   */
  protected async findById(id: string | number, include?: any, omit?: any): Promise<TModel | null> {
    try {
   ...
   * Reusable across all services that extend BaseService
   */
